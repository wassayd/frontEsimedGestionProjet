<script>
    ...
    axios.get("https://localhost:5001/api/Tasks/"+params.guid)
    .then(res => {
        console.log(res);
        task = Object.assign(new Task,res.data)
    })
    .catch(err=>{
        console.log(err)
    })
    ...
    function createTask(){
        let url = "https://localhost:5001/api/Tasks/"+task.id;
        axios.put(url, {
...
{#if projects.length > 0 && users.length > 0}
    <div class="max-w-xl mx-auto py-12 divide-y md:max-w-4xl">
        <div class="py-12">
            <h2 class="text-2xl font-bold">Modifier une tâche</h2>
            <div class="mt-8 max-w-md">
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <label class="block">
                        <button on:click={createTask}  class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Modifier</button>
                    </label>
                </div>
            </div>
        </div>
